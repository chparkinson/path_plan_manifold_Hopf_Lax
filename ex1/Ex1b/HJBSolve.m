function [u,x,p,howManyIter] = HJBSolve(x_target,xf,t,J,sig,tau,kappa,max_iter,tol,a)
% INPUT:
...
%   a - the amplitude of the sin / cos term in M 
% OUTPUT:
...
gd_steps = 1;
gd_rate = 0.025*(a<1.5) + 0.001*(a>=1.5);
...
for k = 1:max_iter
    ...
    for j = 2:J+1
        ...
        %% EXAMPLE 1(b): M = a*sin(pi*x)*cos(pi*y)
        GM(:,j) = a*pi*[cos(pi*x(1,j))*cos(pi*x(2,j));-sin(pi*x(1,j))*sin(pi*x(2,j))]; %gradient of manifold func
        HM(:,:,j) = -a*pi^2*[sin(pi*x(1,j))*cos(pi*x(2,j)),cos(pi*x(1,j))*sin(pi*x(2,j));
